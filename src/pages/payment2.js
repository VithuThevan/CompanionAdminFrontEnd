...
class payment2 extends Component {
  ...
  async componentDidMount() {
    //driver
    const res = await axios.get("http://127.0.0.1:8000/api/payment2");
    console.log(res);
    this.setState({
    ...
    if (this.state.loading) {
      ...
      company_HTML_Table = this.state.company.map((List) => {
        const renderAuthButton = () => {
          if (List.File === null) {
            return "Image has not been uploaded yet";
          } else {
            return (
              <div>
                <img
                ...
                <br />
                <a
                  href={`http://127.0.0.1:8000/${List.File}`}
                  download={"Photo"}
                >
                  <br />
          ...
          <div className="col-md-12">
            <div className="card">
              <div className="card-header">
                <h4>Details of Rejected Payments</h4>
              </div>
